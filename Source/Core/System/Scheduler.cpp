...
public:
    ...
    Scheduler& GetScheduler(const boost::asio::io_context& io_context) {
		for (int i = 0; i < counter_.load(std::memory_order_relaxed); ++i) {
			if (&schedulers_[i].scheduler->GetIoContext() == &io_context) {
				return *schedulers_[i].scheduler;
			}
		}
...
Scheduler& Scheduler::Get(const boost::asio::io_context& io_context) {
    return SchedulerStorage::GetInstance().GetScheduler(io_context);
}
...
void Scheduler::Run() {
    while (is_running_) {
        io_context_->run_for(std::chrono::milliseconds(60));
    }
}
